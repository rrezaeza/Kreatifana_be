...
const getProductDetails = async (whereClause, includeReviews = true) => {
    const product = await prisma.product.findUnique({
        ...
        include: {
            ...
            reviews: includeReviews ? { // Hanya include reviews jika diperlukan
    ...
    const avgRating = includeReviews && product.reviews.length
...
const getProducts = async (req, res, next) => {
    try {
        ...
        // build where clause
        const where = {};
        // Filter published: default true, bisa disetel false jika admin
        if (req.query.published !== undefined) {
            where.published = req.query.published === "true";
        } else if (!(req.user && req.user.isAdmin)) { // Jika bukan admin, hanya tampilkan yang published
...
const createProduct = async (req, res, next) => {
    ...
    try {
        ...
        const thumbnailUrl = req.files && req.files.thumbnail && req.files.thumbnail[0]
            ? `/uploads/${req.files.thumbnail[0].filename}`
            : null;

        const fileUrl = req.files && req.files.file && req.files.file[0]
            ? `/uploads/${req.files.file[0].filename}`
            : null;

        // Validasi Input Wajib
        if (!title || !description || !price || !slug || !categoryId || !thumbnailUrl) {
            // Hapus file yang sudah terupload jika validasi gagal
            if (thumbnailUrl) {
                await fs.unlink(path.join(uploadsBasePath, path.basename(thumbnailUrl))).catch(e => console.error(`Failed to delete partial thumbnail: ${e.message}`));
            }
            if (fileUrl) {
                await fs.unlink(path.join(uploadsBasePath, path.basename(fileUrl))).catch(e => console.error(`Failed to delete partial file: ${e.message}`));
            }
            return res.status(400).json({ success: false, message: "Please provide all required product details, including a thumbnail." });
        }
        ...
        if (existingProduct) {
            ...
            return res.status(409).json({ success: false, message: "Product with this slug already exists. Please choose a different one." });
        }
        ...
        if (thumbnailUrl) {
            await fs.unlink(path.join(uploadsBasePath, path.basename(thumbnailUrl))).catch(e => console.error(`Failed to delete thumbnail after error: ${e.message}`));
        }
        if (fileUrl) {
            await fs.unlink(path.join(uploadsBasePath, path.basename(fileUrl))).catch(e => console.error(`Failed to delete file after error: ${e.message}`));
        }
        next(err); // Teruskan error ke middleware penanganan error global
    }
};
...
const updateProduct = async (req, res, next) => {
    ...
    try {
        ...
        const prod = await prisma.product.findUnique({
            where: { id },
            select: { userId: true, slug: true, thumbnailUrl: true, fileUrl: true, categoryId: true, price: true, description: true, title: true, published: true },
        });

        if (!prod) {
            // Hapus file baru jika produk tidak ditemukan
            if (newThumbnailUrl) await fs.unlink(path.join(uploadsBasePath, path.basename(newThumbnailUrl))).catch(e => console.error(e));
            if (newFileUrl) await fs.unlink(path.join(uploadsBasePath, path.basename(newFileUrl))).catch(e => console.error(e));
            return res.status(404).json({ success: false, message: "Product not found" });
        }
        ...
        const updated = await prisma.$transaction(async (tx) => {
            ...
            if (newThumbnailUrl && prod.thumbnailUrl) {
                const oldThumbnailPath = path.join(uploadsBasePath, path.basename(prod.thumbnailUrl));
                await fs.unlink(oldThumbnailPath).catch(e => console.error(`Failed to delete old thumbnail: ${e.message}`));
            }
            if (newFileUrl && prod.fileUrl) {
                const oldFilePath = path.join(uploadsBasePath, path.basename(prod.fileUrl));
                await fs.unlink(oldFilePath).catch(e => console.error(`Failed to delete old file: ${e.message}`));
            }
            ...
            if (tags.length > 0 || req.body['tags[]'] !== undefined) { // Cek jika tags sengaja dikirim kosong
        ...
        const newThumbnailUrl = req.files && req.files.thumbnail && req.files.thumbnail[0]
            ? `/uploads/${req.files.thumbnail[0].filename}` : null;
        const newFileUrl = req.files && req.files.file && req.files.file[0]
            ? `/uploads/${req.files.file[0].filename}` : null;

        if (newThumbnailUrl) {
            await fs.unlink(path.join(uploadsBasePath, path.basename(newThumbnailUrl))).catch(e => console.error(`Failed to delete new thumbnail after error: ${e.message}`));
        }
        if (newFileUrl) {
            await fs.unlink(path.join(uploadsBasePath, path.basename(newFileUrl))).catch(e => console.error(`Failed to delete new file after error: ${e.message}`));
        }
...
const deleteProduct = async (req, res, next) => {
    ...
    try {
        ...
        if (product.thumbnailUrl) {
            const thumbnailPath = path.join(uploadsBasePath, path.basename(product.thumbnailUrl));
            await fs.unlink(thumbnailPath).catch(e => console.error(`Failed to delete thumbnail ${thumbnailPath}: ${e.message}`));
        }
        if (product.fileUrl) {
            const filePath = path.join(uploadsBasePath, path.basename(product.fileUrl));
            await fs.unlink(filePath).catch(e => console.error(`Failed to delete file ${filePath}: ${e.message}`));
        }
...
const incrementDownload = async (req, res, next) => {
    try {
        ...
        if (!purchase && !isAdmin && !isOwner) {
            return res.status(403).json({ success: false, message: "You must purchase this product before downloading" });
        }
